...
def sum_risk_level(_array):
    risk_level = 0
    cols_num = len(_array[0])
    rows_num = len(_array)
    for y in range(rows_num):
        for x in range(cols_num):
            number = int(_array[y][x])
            has_top = True
            has_bottom = True
            has_left = True
            has_right = True
            ...
            if y == 0:
                ...
                has_bottom = False
            # print(x, y)
            if ((not has_left or number < int(_array[y][x-1])) and
                (not has_right or number < int(_array[y][x+1])) and
                (not has_top or number < int(_array[y-1][x])) and
                (not has_bottom or number < int(_array[y+1][x]))):
                risk_level += (number + 1)
                # print('found:', x, y)
    return risk_level

def gen_lava_dict(_input):
    lava_dict = {}
...
def process_solution(_contents):
    ...
    lava_matrix = np.array(_contents)
    return sum_risk_level(lava_matrix)

@common.elapsed_time_factory()
def process_solution2(_contents):
    # print('Contents:', _contents)
    min_x_y, max_x_y, lava_dict = gen_lava_dict(_contents)
